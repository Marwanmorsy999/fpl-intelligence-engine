#!/usr/bin/env python
"""scripts/verify_live_rss.py — Phase 9.7 Live RSS Feed Verification CLI.

Fetches news from a **real RSS feed**, verifies that the feed is accessible
and correctly parsed into :class:`RawItem` objects, and verifies that the news
is correctly ingested into the Phase 9.2 pipeline (``ingest_raw_text``).

Usage::

    python scripts/verify_live_rss.py
    python scripts/verify_live_rss.py --rss-url https://www.bbc.co.uk/sport/football/teams/rss
    python scripts/verify_live_rss.py --rss-url https://... --dry-run
    python scripts/verify_live_rss.py --rss-url https://... --provider real --db ./fpl.db

``--dry-run`` still fetches from the live feed and runs the whole pipeline, but
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Phase 9.7 — fetch a live RSS feed, verify it parses, and verify "
            "the news is ingested into the Phase 9.2 pipeline."
        ),
    ...
    parser.add_argument(
        "--source-id",
        default="rss_feed",
        help="Phase 9.2 source identifier (default: rss_feed).",
    )
    parser.add_argument(
        "--limit",
        type=int,
        default=20,
        help="Maximum number of items fetched from the feed (default: 20).",
    )
    parser.add_argument(
        "--provider",
        choices=["mock", "real"],
...
def _print_report(report: LiveSourceVerification, *, dry_run: bool) -> None:
    print("=" * 78)
    print("PHASE 9.7 — LIVE RSS FEED VERIFICATION")
    print("=" * 78)
