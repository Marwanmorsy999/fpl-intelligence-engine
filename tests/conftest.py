...
def populated_db(db_session: Session) -> Session:
    ...
    for first, second, web, pos, team_idx in player_data:
        ...
        db.add(
            PlayerExternalId(
                player_id=player.id,
                provider="mock",
                provider_player_id=f"mock_player_{web.lower()}",
            )
        )
        db.add(
            PlayerTeamMembership(
    ...
    for gw_idx, gw in enumerate(gameweeks):
        f1 = Fixture(
            season_id=season.id,
            provider_fixture_id=fixture_idx + 1,
            gameweek_id=gw.id,
            kickoff_time=datetime(2025, 8, 1, tzinfo=UTC) + timedelta(days=gw_idx * 7, hours=3),
            home_team_id=teams[0].id,
            away_team_id=teams[1].id,
            home_score=2 if gw_idx < 2 else None,
            away_score=1 if gw_idx < 2 else None,
            status="completed" if gw_idx < 2 else "scheduled",
            postponed=False,
        )
        db.add(f1)
        fixtures.append(f1)
        ...
        f2 = Fixture(
            ...
            home_score=1 if gw_idx < 2 else None,
            away_score=3 if gw_idx < 2 else None,
            status="completed" if gw_idx < 2 else "scheduled",
            postponed=False,
        )
        db.add(f2)
        ...
        for player_idx, player in enumerate(players):
            ...
            db.add(
                PlayerGameweekPerformance(
                    ...
                    bps=20 + player_idx * 5,
                    influence=50.0 - player_idx * 5,
                    creativity=30.0 - player_idx * 3,
                    threat=40.0 - player_idx * 4,
                    ict_index=12.0 - player_idx,
