...
def test_compare_with_real_catalog_ids(client):
    ...
    assert (
        data["player_a"]["expected_points"] is not None
        or data["player_b"]["expected_points"] is not None
    )
    # diff highlights
...
def test_chips_respects_used_chips(client):
    ...
    assert r.status_code == 200
    r2 = client.get(
        "/api/v1/chips/plans", params={"session_id": "no_chips", "start_gw": 2, "horizon": 8}
    )
...
def test_drawer_shows_set_piece_chips(client):
    ...
    for pid, _should_have in [
        ...
        assert any(flags.get(k) is True for k in ("penalty", "corners", "free_kicks")), (
            f"{pid} should be taker"
        )
