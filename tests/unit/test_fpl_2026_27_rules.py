...
def test_2026_27_rules_file_loads_and_uses_official_goal_points() -> None:
    ...
    assert engine.compute(FPLPointsComponents(expected_goals=1.0), position_code=1)[
        "goals"
    ] == pytest.approx(10.0)
    assert engine.compute(FPLPointsComponents(expected_goals=1.0), position_code=2)[
        "goals"
    ] == pytest.approx(6.0)
    assert engine.compute(FPLPointsComponents(expected_goals=1.0), position_code=3)[
        "goals"
    ] == pytest.approx(5.0)
    assert engine.compute(FPLPointsComponents(expected_goals=1.0), position_code=4)[
        "goals"
    ] == pytest.approx(4.0)
