...
SQUAD = {
    ...
    "gameweek": 2,
    "player_positions": {
        str(i): (1 if i == 1 else 2 if i <= 5 else 3 if i <= 10 else 4) for i in range(1, 16)
    },
...
VALUATION = {
    ...
    "valuation": {
        ...
        "gaps": [],
        "per_gw": [
            {"gw": 2, "xpts_in": 7.4, "xpts_out": 2.0, "delta": 5.4},
            {"gw": 3, "xpts_in": 6.0, "xpts_out": 2.5, "delta": 3.5},
            {"gw": 4, "xpts_in": 5.0, "xpts_out": 1.0, "delta": 4.0},
        ],
        "gross_ev": 12.9,
        "hit_cost": 0,
        "net_ev": 12.9,
        "note": "EV over GWs [2,3,4]",
    ...
    "hit_analysis": {
        ...
        "net_ev": 12.9,
        "recommendation": "EXECUTE",
        "chip_text": "Cost: 0 pts (free transfer). Projected 3-week gain: +12.9 pts. Net EV: +12.9. Recommendation: EXECUTE.",
    },
    "how_computed": "FT Valuation = SUM(xPTS_in - xPTS_out) over next 3 GWs - hit cost",
}
...
SHADOW = {
    ...
    "shadow": {
        ...
        "metrics": {
            str(i): {"xpts": 3.0, "alpha": 0.5, "edge": 0.5, "own_p": 0.1} for i in range(1, 16)
        }
        | {"99": {"xpts": 7.4, "alpha": 3.1, "edge": 4.2, "own_p": 0.12}},
        "valuation": VALUATION["valuation"],
        "xi_xpts_current": 30.0,
        "xi_xpts_shadow": 35.0,
        "xi_delta": 5.0,
        "gameweek": 2,
        "how_computed": "shadow",
...
VALUATION_HIT = {
    ...
    "hit_analysis": {
        ...
        "net_ev": 5.0,
        "recommendation": "EXECUTE",
        "chip_text": "Cost: -4 pts. Projected 3-week gain: +9 pts. Net EV: +5. Recommendation: EXECUTE.",
    },
}
...
TRAJECTORY = {
    ...
    "series": [
        ...
        },
        {
            ...
            "is_you": False,
            "current_total": 105,
            "points": [105, 125, 145, 165],
            "per_gw": [20, 20, 20],
        },
    ],
    "ranks": [
        {"794561": 4, "201": 1, "202": 2, "203": 3},
        {"794561": 2, "201": 1, "202": 3, "203": 4},
        {"794561": 1, "201": 2, "202": 3, "203": 4},
        {"794561": 1, "201": 2, "202": 3, "203": 4},
    ],
...
FOMO = {
    ...
    "captain_regret": {
        "gameweek": 2,
        "recommended_captain": 99,
        "recommended_points": 12,
        "user_captain": 3,
        "user_points": 6,
        "delta": 12,
        "line": "You lost 12 pts by ignoring the captain recommendation. You captained #3 (6 pts) vs engine pick #99 (12 pts doubled).",
        "how_computed": "Captain regret = (engine captain actual pts x2) - (your captain actual pts x2)",
    },
    "alpha_capture": {
        "gameweek": 2,
        "graded_transfers": 4,
        "right": 3,
        "rate": 0.75,
        "line": "Alpha Capture Rate: 75% of graded transfer calls were right",
        "how_computed": "right/(right+wrong)",
    },
...
def mocked(page: Page) -> Iterator[Page]:
        ...
        _json(
            {
                ...
                "starting_xi": [1, 2, 3],
                "bench_order": [],
                "captain": {"player_id": 3, "expected_points": 7.4},
                "players": {},
            }
        ),
    )
    ...
    page.route(
        "**/api/v1/players**",
        _json(
            [{"id": i, "fpl_element_id": i, "web_name": f"Player{i}"} for i in range(1, 20)]
            + [
                {"id": 99, "fpl_element_id": 99, "web_name": "Saka"},
                {"id": 100, "fpl_element_id": 100, "web_name": "Haaland"},
            ]
        ),
    )
    # Phase 27
...
class TestTransferDesk:
    ...
    def test_execute_shows_success(self, mocked: Page):
        ...
        expect(page.locator('[data-testid="execute-success"]')).to_contain_text(
            "Transfer Executed on FPL"
        )
    ...
    def test_hit_cost_chip_minus_four(self, mocked: Page):
        ...
        expect(page.locator('[data-testid="hit-cost-chip"]')).to_contain_text(
            "Projected 3-week gain: +9 pts."
        )
...
class TestLeagueTrajectory:
    def test_trajectory_chart_renders(self, mocked: Page):
        ...
        # insight line
        expect(page.locator('[data-testid="trajectory-insight"]')).to_contain_text(
            "14 pts behind SuperBata"
        )
        expect(page.locator('[data-testid="trajectory-insight"]')).to_contain_text(
            "Projected to overtake by GW4"
        )
    ...
    def test_fomo_math_asserts(self, mocked: Page):
        ...
        expect(page.locator('[data-testid="fomo-cost-line"]').first).to_contain_text(
            "You lost 12 pts", timeout=8000
        )
