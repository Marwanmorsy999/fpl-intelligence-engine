...
PLAYER_FIRST_NAMES = [
    "Mohamed", "Kevin", "Harry", "Erling", "Bukayo",
    "Martin", "Bruno", "Son", "James", "Phil",
    "Marcus", "Gabriel", "William", "Jack", "Declan",
    "Trent", "Andrew", "Kieran", "Ben", "Aaron",
    "Jordan", "John", "David", "Thomas", "Kai",
    "Cole", "Michael", "Chris", "Daniel", "Ryan",
]

PLAYER_SECOND_NAMES = [
    "Salah", "De Bruyne", "Kane", "Haaland", "Saka",
    "Odegaard", "Fernandes", "Heung-min", "Maddison", "Foden",
    "Rashford", "Jesus", "Saliba", "Grealish", "Rice",
    "Alexander-Arnold", "Robertson", "Tierney", "White", "Ramsdale",
    "Pickford", "Stones", "Silva", "Partey", "Havertz",
    "Palmer", "Wood", "Wilson", "James", "Fraser",
]
...
class MockHistoricalDataProvider:
    ...
    def _generate_data(self) -> None:
        for season_index, season_name in enumerate(SEASONS):
            ...
            for player_index in range(30):
                ...
                players.append({
                    "provider_player_id": _player_id(self._provider_name, season_index, player_index),
                    "first_name": PLAYER_FIRST_NAMES[player_index % len(PLAYER_FIRST_NAMES)],
                    "second_name": PLAYER_SECOND_NAMES[player_index % len(PLAYER_SECOND_NAMES)],
                    "web_name": f"{PLAYER_FIRST_NAMES[player_index % len(PLAYER_FIRST_NAMES)][0]}. {PLAYER_SECOND_NAMES[player_index % len(PLAYER_SECOND_NAMES)]}",
            ...
            for gw in range(1, 39):
                for match in range(0, len(TEAM_NAMES) // 2):
                    home_idx = (fixture_index * 2) % len(TEAM_NAMES)
                    away_idx = (fixture_index * 2 + 1) % len(TEAM_NAMES)
                    kickoff = datetime(2022 + season_index, 8, 1, 15, 0, tzinfo=UTC) + timedelta(days=fixture_index * 7)
                    fixtures.append({
                        "provider_fixture_id": _fixture_id(self._provider_name, season_index, fixture_index),
                        "gameweek": gw,
            ...
            for p_idx, player in enumerate(players):
                for gw in range(1, 39):
                    fpl_history.append({
                        ...
                        "red_cards": 0,
                        "saves": 3 if POSITIONS[p_idx % len(POSITIONS)] == 1 and gw % 2 == 0 else 0,
                        ...
                        "expected_goal_involvements": 0.5 if (gw % 5 == 0 or gw % 7 == 0) else 0.0,
            ...
            for p_idx, player in enumerate(players):
                for gw in range(1, 39):
                    snap_time = datetime(2022 + season_index, 8, 1, 18, 0, tzinfo=UTC) + timedelta(days=(gw - 1) * 7)
    ...
    def get_player_match_stats(self, season: str, player_id: str) -> Sequence[Mapping[str, object]]:
        ...
        if data is None:
            return []
        return [
            s for s in data["player_match_stats"]
            if s["provider_player_id"] == player_id
        ]
