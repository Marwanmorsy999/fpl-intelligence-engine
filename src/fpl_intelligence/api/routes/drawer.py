    ...
    try:
        ...
        if is_schema_error:
            ...
            try:
                ...
                logger.warning(
                    "element_facts fallback SELECT after seal failed for %s: %s", player_id, exc2
                )
                with contextlib.suppress(Exception):
                    db.rollback()
...
def _load_prediction_materialized(db: Session, player_id: int, gw: int) -> dict[str, Any] | None:
    ...
    try:
        ...
        return {
            ...
            "minutes_estimate": float(row.minutes_estimate)
...
def _form_bars_from_history(db: Session, player_id: int) -> list[dict[str, Any]]:
    ...
    try:
        ...
        return [
            {"gw": gw, "points": points, "minutes": minutes} for gw, points, minutes in sorted(rows)
        ]
    except Exception as exc:
...
async def player_drawer(
    ...
    if (not fixture_runs or len(fixture_runs) < HORIZON_GWS) and not fixture_runs:
            ...
            try:
                ...
                if urow is not None:
                    ...
                    if xg is None:
                        missing.append(
                            "understat_unmatched"
                        ) if "understat_unmatched" not in missing else None
            except Exception as exc:
    ...
    generated_at = (
        fetched_at.isoformat() if fetched_at is not None else datetime.now(UTC).isoformat()
    )
