...
def parse_players_raw_csv(text: str) -> dict[int, dict[str, Any]]:
    ...
    for row in _rows(text):
        ...
        facts[element_id] = {
            ...
            # Additional FPL fields for compatibility
            "chance_of_playing_next_round": _opt_int(row.get("chance_of_playing_next_round")),
            "chance_of_playing_this_round": _opt_int(row.get("chance_of_playing_this_round")),
            "element_type": _opt_int(row.get("element_type")),
            "ep_next": _opt_float(row.get("ep_next")),
            "ep_this": _opt_float(row.get("ep_this")),
            "transfers_in_event": _opt_int(row.get("transfers_in_event")),
            "transfers_out_event": _opt_int(row.get("transfers_out_event")),
            "transfers_in_season": _opt_int(row.get("transfers_in")),
            "transfers_out_season": _opt_int(row.get("transfers_out")),
            "total_points": _opt_int(row.get("total_points")),
            "points_per_game": _opt_float(row.get("points_per_game")),
            "form": _opt_float(row.get("form")),
            "ict_index": _opt_float(row.get("ict_index")),
            "goals_scored": _opt_int(row.get("goals_scored")),
            "assists": _opt_int(row.get("assists")),
            "clean_sheets": _opt_int(row.get("clean_sheets")),
            "yellow_cards": _opt_int(row.get("yellow_cards")),
            "red_cards": _opt_int(row.get("red_cards")),
            "bonus": _opt_int(row.get("bonus")),
